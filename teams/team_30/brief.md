...
**Your domain:** Academic freedom vs. regime: whether v2clacfree predicts SSH content better than v2x_libdem, testing mechanism specificity
**Sub-family label (use exactly this for theory_family):** `regime-channels`
**Complexity:** Simple
**Permitted measurement approach:** Parallel models using v2clacfree and v2x_libdem; compare coefficients and model fit — no external API required

## Self-censorship theory (shared by all 30 teams)
...
Each team formulates and pre-registers **exactly one core hypothesis** -- the single most tractable and theoretically grounded implication of the self-censorship theory within your domain. Robustness checks re-test the same hypothesis with alternative specifications; they are not additional hypotheses.
...
- Write to `teams/team_30/analysis_plan.md`:
...
- Write R code in `teams/team_30/analysis/analysis.R` (tidyverse style)
...
- Produce 2-4 figures or tables; save to `teams/team_30/analysis/figures/`
- Save a `teams/team_30/analysis/primary_results.json` with fields:
...
- Write a 4-5 page report to `teams/team_30/report/report.md`
