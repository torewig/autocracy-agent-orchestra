# Research Team 01 -- Brief [PILOT — not for final analysis]
...
- `data/agent_corpus.rds` -- SSH articles from Web of Science, **1945-1983**
  (Cold War era legacy corpus), merged with V-DEM regime data. One row = one
  article x one author-country. ~202,000 articles / ~207,000 rows total.
- `DATA/vdem/vdem_clean.rds` -- V-DEM country-year data (standalone).
- `data/vdem_codebook.md` -- variable definitions and guidance.

Key data notes:
- **Coverage: 1945-1983 only.** Do not assume post-Cold War data exists.
- Primary regime measure: `v2x_libdem` (continuous, 0-1). Use as main IV.
  Report at least one robustness check using `v2x_regime` or `regime_binary`.
- Citations: Do not use `tot_cites` raw. Use `field_year_mean_cites` to
  normalize, or restrict comparisons within field.
- Volume analyses: Use `n_articles_country_year` as denominator.
- Pre-1970 data: Very sparse; treat with caution.
- Regime distribution heavily skewed to liberal democracies (USA dominates).
- Key V-DEM variables also available: `v2clacfree` (academic freedom),
  `v2x_freexp_altinf` (freedom of expression), `v2csreprss` (CSO repression),
  `v2xnp_regcorr` (neopatrimonialism).
...
- Write to `teams/team_01/analysis_plan.md`:
...
- Write R code in `teams/team_01/analysis/analysis.R` (tidyverse style)
...
- Produce 2-4 figures or tables; save to `teams/team_01/analysis/figures/`
- Include at least one robustness check using an alternative regime measure
- Write `teams/team_01/analysis/primary_results.json` with the primary
...
- Write a 4-5 page report to `teams/team_01/report/report.md`
